...
#define CET_WString_init 		CET_BasicString_init(wchar_t)
...
#define CET_BasicString_pop_back(vec) do { \
    if ((str).length) \
        (str).length--; \
} while (0)
...
#define CET_BasicString_init(T) \
	{CET_calloc(sizeof(T), CET_BASICSTRING_INIT_CAPACITY), \
	 0, CET_BASICSTRING_INIT_CAPACITY}
