...
static size_t str_destructor_counter = 0;

static void str_destructor(STR_TYPE *element)
{
	str_destructor_counter++;
}
...
TEST destroy(void)
{
	BasicString(STR_TYPE) str = BasicString_init(STR_TYPE, 0);
...
TEST compare(void)
{
	BasicString(STR_TYPE) str1 = BasicString_init(STR_TYPE, 0);
	BasicString(STR_TYPE) str2 = BasicString_init(STR_TYPE, 0);
...
{
	BasicString(STR_TYPE) str = BasicString_init(STR_TYPE, str_destructor);

	BasicString_assign(&str, TABLE);

	BasicString_destroy(&str);

	ASSERT(str_destructor_counter, STR_SIZE);
}
